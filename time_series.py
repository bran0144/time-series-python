...
# Cointegration Modelss
# even if the prices of two different assets both follow random walks, it is still possible
#   that a linear combination of them is not a random walk
# THis means that the linear combination could be forecastable (even if the two separate aren't)
#   this is called cointegrated
# Walking the dog on a leash analogy
# Two steps to test for cointegration:
#   Regress one against the toher to get slope
#   Then, run the Augmented Dickey-Fuller test (Pt - cQt) to test for random walk
# statsmodels also has a coint function that combines these steps
...
# forecase temp over next 30 years
